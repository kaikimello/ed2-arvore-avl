...
public class ArvoreAVL<G extends Comparable<G>> {
	...
	private int calcularAltura(NoAVL<G> no) {
		...
		if (no.getDireito() != null){
			alturaDireita = no.getDireito().getAltura();
		}
		 ...
		 */
		if (no.getEsquerdo() != null){
			alturaEquerda = no.getEsquerdo().getAltura();
		}

		/*
		 * Retornar o valor da maior altura dos seus filhos mais um.
		 */
		return 1 + Math.max(alturaDireita, alturaEquerda);
	}
	...
	private int fatorBalanceamento(NoAVL<G> no) {
		 ...
		 */
		if (no.getEsquerdo() != null){
			alturaEsquerda = no.getEsquerdo().getAltura();
		}
	...
	private NoAVL<G> rotacaoDireita(NoAVL<G> alvo) {
		 ...
		 */
		alvo.setAltura(this.calcularAltura(alvo));
		substituto.setAltura(this.calcularAltura(substituto));
	...
	private NoAVL<G> balanceamento(NoAVL<G> no) {
		...
		if (fb == +2) {
			...
			if (this.fatorBalanceamento(no.getDireito()) < 0) {
				no.setDireito(this.rotacaoDireita(no.getDireito()));
			}
			...
			if (this.fatorBalanceamento(no.getEsquerdo()) > 0) {
				no.setEsquerdo(this.rotacaoEsquerda(no.getEsquerdo()));
			}
